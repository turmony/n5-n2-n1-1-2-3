...
**架构：** 使用 Python 3.10 命令行程序管理 jlpt-notes/ 资料库。卡片采用 YAML 元数据加 Markdown 正文；复习和答题使用仅追加的 JSONL 日志。Codex 负责检查学习输出与编写原创题目；程序负责校验、持久化、调度、计分与统计。

**技术栈：** Python 3.10、标准库、PyYAML 6.x、pytest、本地 Git、Codex 定时任务。
